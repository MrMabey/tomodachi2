var contentOld = function() {
  ...
  const definition = defineContentScript({
    ...
    async main() {
      ...
      if ((gameState == null ? void 0 : gameState.isHatched) && (gameState == null ? void 0 : gameState.selectedEgg)) {
      ...
      async function awardTokens(amount, reason) {
        try {
          ...
          if (floatingAvatar && floatingAvatar.setMood) {
            ...
            setTimeout(() => floatingAvatar.setMood("content"), 2e3);
          }
        } catch (error) {
      ...
      function createFloatingAvatar(avatarType) {
      ...
      `;
        ...
        if (ctx) {
          ...
          const animate = () => {
            ...
            ctx.scale(breathe * 0.5, breathe * 0.5);
            if (avatarType === "cat") {
              ...
              drawSimpleFloatingAlien(ctx);
            } else if (avatarType === "panda") {
              drawSimpleFloatingPanda(ctx);
            }
            ctx.restore();
      ...
      function drawSimpleFloatingCat(ctx, time) {
      ...
      function drawSimpleFloatingDragon(ctx, time) {
      ...
      function drawSimpleFloatingRobot(ctx, time) {
      ...
      function drawSimpleFloatingGhost(ctx, time) {
      ...
      function drawSimpleFloatingAlien(ctx, time) {
      ...
      function drawSimpleFloatingPanda(ctx, time) {
      ...
      chrome.storage.onChanged.addListener((changes) => {
        if (changes["floavatar-gamestate"]) {
          ...
          if ((newState == null ? void 0 : newState.isHatched) && (newState == null ? void 0 : newState.selectedEgg) && !floatingAvatar) {
            createFloatingAvatar(newState.selectedEgg);
          }
          if (!(newState == null ? void 0 : newState.isHatched) && floatingAvatar) {
            const existing = document.getElementById("floavatar-floating");
            if (existing) existing.remove();
            floatingAvatar = null;
          }
  ...
  const result = (async () => {
    try {
      ...
      logger.error(
        `The unlisted script "${"content-old"}" crashed on startup!`,
        err
      );
      throw err;
    }
