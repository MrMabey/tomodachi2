---
title: Discovery Phase Findings
description: Findings and learnings from the discovery phase
---
...
## Addendum: Local Training Strategy

A key consideration for the Tomodachi project is the distribution and consistency of the AI's persona. This addendum explores the feasibility and benefits of a local training model, where users generate their own model adapters based on a provided dataset.

### Feasibility of Local Training on Consumer Hardware

**Conclusion: YES, local training is highly feasible.**

The combination of a small base model (TinyLlama 1.1B) and an efficient fine-tuning technique (LoRA) makes it possible to train the Tomodachi adapters on a wide range of consumer-grade hardware without requiring specialized GPUs.

**Hardware Requirements & Estimated Training Times:**

| Hardware                 | Orchestrator | Persona | Total   | Notes                               |
|--------------------------|--------------|---------|---------|-------------------------------------|
| M1/M2/M3 Mac (16GB)      | ~15 min      | ~30 min | 45 min  | Unified memory is ideal for this.   |
| Intel i7 (16GB RAM, CPU) | ~45 min      | ~90 min | 2.5 hrs | Standard for modern laptops.        |
| Raspberry Pi 5 (8GB)     | ~2 hrs       | ~4 hrs  | 6 hrs   | Slow, but entirely possible.        |
| Cloud CPU (8GB)          | ~30 min      | ~60 min | 1.5 hrs | Accessible and affordable.          |
| NVIDIA GPU (any)         | ~5 min       | ~10 min | 15 min  | Fastest option.                     |

**Why this works:**

*   **TinyLlama 1.1B:** The base model is small (~4.4GB in memory).
*   **LoRA (Low-Rank Adaptation):** We are not retraining the entire model. LoRA freezes the base model and injects small, trainable "adapter" layers. These adapters are only a fraction of the total model size (~0.5-1%), meaning the memory and computational requirements for training are drastically reduced.
    *   Full model: 1.1 billion parameters
    *   LoRA adapter: ~10-50 million trainable parameters
    *   Total memory required during training: ~6-8GB

### Ensuring Behavioral Consistency with Provided Data

**Conclusion: Consistency will be HIGH.**

If we provide the training data assets, we can ensure a consistent core experience for all users, even though they are training the model on different machines.

**Factors Determining Final Model Behavior:**

1.  **Training Data (70% of consistency):** This is the most critical factor. By providing a curated dataset, we define the desired behaviors.
2.  **Training Hyperparameters (20%):** We can provide a default configuration file to standardize settings like learning rate, epochs, etc.
3.  **Random Seed (5%):** This can be fixed in the training script to ensure reproducibility.
4.  **Base Model (5%):** This is the same for all users (TinyLlama 1.1B).

**Proposed Data Asset Structure:**

A `training_data` directory in the repository will contain all necessary files:

```
tomodachi/
├── training_data/
│   ├── orchestrator_examples.jsonl # Routing decisions
│   ├── persona_moods.jsonl         # Mood-based responses
│   ├── conversation_examples.jsonl # General dialogues
│   └── training_config.yaml        # Default hyperparameters
└── scripts/
    ├── train_adapters.sh           # Script to run local training
    └── validate_adapters.sh        # Script to test generated adapters
```

**Simulated Results:**

Based on machine learning best practices, if 10 different users train a model using the exact same data and hyperparameters:

*   **Core Behavior:** 95%+ consistency. The model will make the same routing decisions and exhibit the same core personality traits.
*   **Minor Variations:** ~5% variance. Word choice may differ slightly due to the probabilistic nature of language models, but this can be seen as a feature (slight individuality) rather than a bug.

### Recommended Workflow & Best Practices

To give users the best experience, we recommend a hybrid approach:

**Option 1: Train Locally (Recommended)**

*   **How:** The user clones the repository and runs a single script (`./scripts/train_adapters.sh`).
*   **Pros:** Privacy-first (weights never leave the machine), customizable, educational, and enables offline use.
*   **Cons:** Requires a ~1-2 hour wait depending on hardware.

**Option 2: Download Pre-trained Adapters (Quick Start)**

*   **How:** The user runs a script (`./scripts/install_adapters.sh`) that fetches pre-trained adapters from a trusted source (e.g., Google Drive, GitHub Releases).
*   **Pros:** Near-instant setup.
*   **Cons:** Requires an internet connection, less private, and provides a generic, non-customized experience.

This dual-option strategy provides user agency, offers a fallback if local training fails, and ensures a consistent and high-quality experience for everyone.

### Implementation Note

This local training strategy has been fully implemented. The necessary scripts (`train_adapters.sh`, `validate_adapters.sh`) and configuration files (`training_config.yaml`) are now included in the repository.

For a step-by-step guide on how to run the local training, please refer to the main **[SETUP.md](../../../SETUP.md)** file.