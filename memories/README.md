...
1. **Create the virtual environment** (Homebrew Python 3.11 by default):
   ```bash
   ./scripts/setup_venv.sh           # or: PYTHON_BIN=/path/to/python3.11 ./scripts/setup_venv.sh
   source .venv/bin/activate
   ```
   _Manual alternative_: `/opt/homebrew/bin/python3.11 -m venv .venv` followed by `pip install -r requirements.txt`.
2. **Run the pipeline tests**:
   ```bash
   python -m unittest discover -s tests
   ```
3. **Deactivate when finished**: `deactivate` (Reactivate with `source .venv/bin/activate`).
